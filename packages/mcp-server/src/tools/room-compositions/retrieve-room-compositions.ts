...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  const { room_composition_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.roomCompositions.retrieve(room_composition_id)),
    );
  } catch (error) {
    if (isJqError(error)) {
