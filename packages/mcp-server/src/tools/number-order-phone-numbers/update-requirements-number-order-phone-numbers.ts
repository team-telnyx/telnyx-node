...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['number_order_phone_number_id'],
    $defs: {
...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  const { number_order_phone_number_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.numberOrderPhoneNumbers.updateRequirements(number_order_phone_number_id, body),
      ),
    );
  } catch (error) {
    if (isJqError(error)) {
