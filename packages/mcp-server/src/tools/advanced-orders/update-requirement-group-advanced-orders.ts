...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  const { 'advanced-order-id': advanced_order_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.advancedOrders.updateRequirementGroup(advanced_order_id, body),
      ),
    );
  } catch (error) {
