...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.wireless.detailRecordsReports.delete(id)),
    );
  } catch (error) {
    if (isJqError(error)) {
