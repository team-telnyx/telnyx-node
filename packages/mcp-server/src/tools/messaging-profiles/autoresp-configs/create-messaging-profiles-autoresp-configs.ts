...
export const tool: Tool = {
  name: 'create_messaging_profiles_autoresp_configs',
  description:
  ...
  inputSchema: {
    ...
    required: ['profile_id', 'country_code', 'keywords', 'op'],
  },
  annotations: {},
};
...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.messagingProfiles.autorespConfigs.create(profile_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
