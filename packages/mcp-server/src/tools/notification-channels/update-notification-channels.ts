...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['notification_channel_id'],
  },
  annotations: {},
};

export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  const { notification_channel_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.notificationChannels.update(notification_channel_id, body)),
    );
  } catch (error) {
