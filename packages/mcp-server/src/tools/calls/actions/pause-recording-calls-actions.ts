...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.calls.actions.pauseRecording(call_control_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
