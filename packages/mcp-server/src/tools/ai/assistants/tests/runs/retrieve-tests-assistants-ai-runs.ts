...
export const tool: Tool = {
  name: 'retrieve_tests_assistants_ai_runs',
  description:
...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  const { run_id, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.ai.assistants.tests.runs.retrieve(run_id, body)),
  );
};
