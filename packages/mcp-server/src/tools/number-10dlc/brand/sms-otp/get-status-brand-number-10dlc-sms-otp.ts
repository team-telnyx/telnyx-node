...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number10dlc.brand.smsOtp.getStatus(referenceId, body)),
    );
  } catch (error) {
