...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.telephonyCredentials.retrieve(id)));
  } catch (error) {
    if (isJqError(error)) {
