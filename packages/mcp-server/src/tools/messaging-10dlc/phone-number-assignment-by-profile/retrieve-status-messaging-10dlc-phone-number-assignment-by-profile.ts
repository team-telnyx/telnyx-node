...
export const tool: Tool = {
  name: 'retrieve_status_messaging_10dlc_phone_number_assignment_by_profile',
  description:
...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.messaging10dlc.phoneNumberAssignmentByProfile.retrieveStatus(taskId),
      ),
    );
  } catch (error) {
