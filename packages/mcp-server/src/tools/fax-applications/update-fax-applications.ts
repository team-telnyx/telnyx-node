...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['id', 'application_name', 'webhook_event_url'],
    $defs: {
      anchorsite_override: {
        type: 'string',
        title: 'Anchorsite Override',
...
export const handler = async (client: Telnyx, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.faxApplications.update(id, body)));
  } catch (error) {
    if (isJqError(error)) {
