...
describe('resource voiceDesigns', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.voiceDesigns.create({ prompt: 'Speak in a warm, friendly tone', text: 'Hello, welcome to our service.' });
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.voiceDesigns.retrieve('id', { version: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.voiceDesigns.list({
  ...
  test.skip('downloadSample: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.voiceDesigns.downloadSample('id', { version: 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Telnyx.NotFoundError);
  });
