...
describe('resource portouts', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.portouts.list({
    filter: {
  ...
  test.skip('listRejectionCodes: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.portouts.listRejectionCodes('329d6658-8f93-405d-862f-648776e8afd7', { filter: { code: 1002 } }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Telnyx.NotFoundError);
  });
  ...
  test.skip('updateStatus: only required params', async () => {
    const responsePromise = client.portouts.updateStatus('authorized', { id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', reason: 'I do not recognize this transaction' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
