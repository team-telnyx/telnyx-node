...
describe('resource pronunciationDicts', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.pronunciationDicts.create({ items: [{
    alias: 'tel-nicks',
    text: 'Telnyx',
    type: 'alias',
  }], name: 'Brand Names' });
  ...
  test.skip('create: required and optional params', async () => {
    const response = await client.pronunciationDicts.create({ items: [{
    alias: 'tel-nicks',
    text: 'Telnyx',
    type: 'alias',
  }], name: 'Brand Names' });
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.pronunciationDicts.list({ 'page[number]': 1, 'page[size]': 1 }, { path: '/_stainless_unknown_path' }))
      .rejects
