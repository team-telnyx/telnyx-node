...
describe('resource embeddings', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.ai.embeddings.list({ status: ['string'] }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('similaritySearch: only required params', async () => {
    const responsePromise = client.ai.embeddings.similaritySearch({ bucket_name: 'bucket_name', query: 'query' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
