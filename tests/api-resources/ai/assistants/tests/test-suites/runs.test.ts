...
describe('resource runs', () => {
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.ai.assistants.tests.testSuites.runs.list('suite_name', {
    'page[number]': 0,
    'page[size]': 0,
    status: 'status',
  ...
  test.skip('trigger: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.ai.assistants.tests.testSuites.runs.trigger('suite_name', { destination_version_id: '123e4567-e89b-12d3-a456-426614174000' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Telnyx.NotFoundError);
  });
});
