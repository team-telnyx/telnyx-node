...
describe('resource actions', () => {
  ...
  test.skip('uploadFile: request options and params are passed correctly', async () => {
    ...
    await expect(client.messagingHostedNumberOrders.actions.uploadFile('id', { bill: await toFile(Buffer.from('Example data'), 'README.md'), loa: await toFile(Buffer.from('Example data'), 'README.md') }, { path: '/_stainless_unknown_path' }))
      .rejects
