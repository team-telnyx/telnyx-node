...
describe('resource sessions', () => {
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.rooms.sessions.retrieve('0ccc7b54-4df3-4bca-a65a-3da1ecc777f0', { include_participants: true }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('list0: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.rooms.sessions.list0({
    filter: {
    active: true,
    date_created_at: {
    eq: '2021-04-25',
  ...
  test.skip('list1: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.rooms.sessions.list1('0ccc7b54-4df3-4bca-a65a-3da1ecc777f0', {
    filter: {
    active: true,
    date_created_at: {
    eq: '2021-04-25',
  ...
  test.skip('retrieveParticipants', async () => {
    const responsePromise = client.rooms.sessions.retrieveParticipants('0ccc7b54-4df3-4bca-a65a-3da1ecc777f0');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('retrieveParticipants: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.rooms.sessions.retrieveParticipants('0ccc7b54-4df3-4bca-a65a-3da1ecc777f0', {
    filter: {
    context: 'Alice',
    date_joined_at: {
    eq: '2021-04-25',
