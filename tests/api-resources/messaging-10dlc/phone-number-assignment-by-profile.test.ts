...
describe('resource phoneNumberAssignmentByProfile', () => {
  ...
  test.skip('assign: only required params', async () => {
    const responsePromise = client.messaging10dlc.phoneNumberAssignmentByProfile.assign({ messagingProfileId: '4001767e-ce0f-4cae-9d5f-0d5e636e7809' });
  ...
  test.skip('listPhoneNumberStatus', async () => {
    const responsePromise = client.messaging10dlc.phoneNumberAssignmentByProfile.listPhoneNumberStatus('taskId');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('listPhoneNumberStatus: request options and params are passed correctly', async () => {
    ...
    await expect(client.messaging10dlc.phoneNumberAssignmentByProfile.listPhoneNumberStatus('taskId', { page: 0, recordsPerPage: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test.skip('retrievePhoneNumberStatus', async () => {
    const responsePromise = client.messaging10dlc.phoneNumberAssignmentByProfile.retrievePhoneNumberStatus('taskId');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('retrievePhoneNumberStatus: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.messaging10dlc.phoneNumberAssignmentByProfile.retrievePhoneNumberStatus('taskId', { page: 0, recordsPerPage: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Telnyx.NotFoundError);
  });
