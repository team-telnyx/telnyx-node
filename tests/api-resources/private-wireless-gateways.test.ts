...
describe('resource privateWirelessGateways', () => {
  ...
  test.skip('create: only required params', async () => {
    const responsePromise = client.privateWirelessGateways.create({ name: 'My private wireless gateway', network_id: '6a09cdc3-8948-47f0-aa62-74ac943d6c58' });
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.privateWirelessGateways.list({
    'filter[created_at]': 'filter[created_at]',
    'filter[ip_range]': 'filter[ip_range]',
    'filter[name]': 'filter[name]',
    'filter[region_code]': 'filter[region_code]',
