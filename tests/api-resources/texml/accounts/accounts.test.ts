...
describe('resource accounts', () => {
  ...
  test.skip('retrieveRecordingsJson: request options and params are passed correctly', async () => {
    ...
    await expect(client.texml.accounts.retrieveRecordingsJson('account_sid', {
  ...
  test.skip('retrieveTranscriptionsJson: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.texml.accounts.retrieveTranscriptionsJson('account_sid', { PageSize: 0, PageToken: 'PageToken' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Telnyx.NotFoundError);
  });
});
