...
describe('resource webhooks', () => {
  test.skip('unwrap', () => {
    ...
    expect(() => {
      const badSig = wh.sign(msgID, timestamp, 'some other payload');
      client.withOptions({publicKey: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-signature': badSig }})
    }).toThrow('No matching signature found');
    expect(() => {
      client.webhooks.unwrap(payload, { headers: {...headers, 'webhook-timestamp': '5' }, key})
    }).toThrow('Message timestamp too old');
    expect(() => {
      client.withOptions({publicKey: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-timestamp': '5' }})
    }).toThrow('Message timestamp too old');
    expect(() => {
      client.webhooks.unwrap(payload, { headers: {...headers, 'webhook-id': 'wrong' }, key})
    }).toThrow('No matching signature found');
    expect(() => {
      client.withOptions({publicKey: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-id': 'wrong' }})
    }).toThrow('No matching signature found');
  });
});
