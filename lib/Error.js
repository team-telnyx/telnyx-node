...
_Error.TelnyxInvalidRequestError = TelnyxError.extend({
  type: 'TelnyxInvalidRequestError',
}); // 400
_Error.TelnyxAuthenticationError = TelnyxError.extend({
  type: 'TelnyxAuthenticationError',
}); // 401
_Error.TelnyxPermissionError = TelnyxError.extend({
  type: 'TelnyxPermissionError',
}); // 403
_Error.TelnyxResourceNotFoundError = TelnyxError.extend({
  type: 'TelnyxResourceNotFoundError',
}); // 404
_Error.TelnyxMethodNotSupportedError = TelnyxError.extend({
  type: 'TelnyxMethodNotSupportedError',
}); // 405
_Error.TelnyxTimeoutError = TelnyxError.extend({type: 'TelnyxTimeoutError'}); // 408
_Error.TelnyxUnsupportedMediaTypeError = TelnyxError.extend({
  type: 'TelnyxUnsupportedMediaTypeError',
}); // 415
_Error.TelnyxInvalidParametersError = TelnyxError.extend({
  type: 'TelnyxInvalidParametersError',
}); // 422
_Error.TelnyxRateLimitError = TelnyxError.extend({
  type: 'TelnyxRateLimitError',
}); // 429
_Error.TelnyxAPIError = TelnyxError.extend({type: 'TelnyxAPIError'}); // 500
_Error.TelnyxServiceUnavailableError = TelnyxError.extend({
  type: 'TelnyxServiceUnavailableError',
}); // 503

_Error.TelnyxConnectionError = TelnyxError.extend({
  type: 'TelnyxConnectionError',
});
_Error.TelnyxSignatureVerificationError = TelnyxError.extend({
