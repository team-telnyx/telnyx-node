...
import { SpeechToTextEmitter, SpeechToTextStreamMessage, WebSocketError, buildURL } from './internal-base';
...
export abstract class SpeechToTextWSBase<TSocket extends WebSocketLike> extends SpeechToTextEmitter {
  url!: URL;
  socket!: TSocket;

  protected _client: Telnyx;
  protected _parameters: Record<string, unknown> | null | undefined;
  private _reconnectOptions: SpeechToTextWSReconnectOptions | null;
  private _sendQueue: SendQueue<SpeechToTextAPI.TranscribeClientEvent>;
  ...
  constructor(
    ...
    options?: SpeechToTextWSBaseOptions | undefined,
  ) {
    super();
    this._client = client;
    this._parameters = undefined;
    this._reconnectOptions = options?.reconnect ?? null;
    this._sendQueue = new SendQueue<SpeechToTextAPI.TranscribeClientEvent>(options?.maxQueueSize);
  }
  ...
  send(event: SpeechToTextAPI.TranscribeClientEvent) {
    if (!this.socket) {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<SpeechToTextStreamMessage> {
    ...
    const queue: SpeechToTextStreamMessage[] = [];
    const resolvers: (() => void)[] = [];
    let done = false;
    let currentSocket = this.socket;

    const push = (msg: SpeechToTextStreamMessage) => {
      queue.push(msg);
      resolvers.shift()?.();
    };

    const onEvent = (event: SpeechToTextAPI.TranscribeServerEvent) => {
    ...
    const resolve = (res: (value: IteratorResult<SpeechToTextStreamMessage>) => void) => {
      if (queue.length > 0) {
    ...
    const next = (): Promise<IteratorResult<SpeechToTextStreamMessage>> =>
      new Promise((res) => {
  ...
  private _connect(): TSocket {
    ...
    socket.on('message', (data: string | ArrayBuffer | ArrayBufferView, isBinary: boolean) => {
      ...
      let event: SpeechToTextAPI.TranscribeServerEvent;
      try {
        event = JSON.parse(text) as SpeechToTextAPI.TranscribeServerEvent;
      } catch {
        this._emit('raw', data);
        return;
      }
