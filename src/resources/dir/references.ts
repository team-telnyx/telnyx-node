...
export class References extends APIResource {
   ...
   * @example
   * ```ts
   * const references = await client.dir.references.list(
   *   '16635d38-75a6-4481-82e8-69af60e05011',
   * );
   * ```
   */
  list(dirID: string, options?: RequestOptions): APIPromise<ReferenceListResponse> {
   ...
   * @example
   * ```ts
   * const response = await client.dir.references.submit(
   *   '16635d38-75a6-4481-82e8-69af60e05011',
   *   {
   *     business_references: [
   *       {
  ...
  submit(
    dirID: string,
    body: ReferenceSubmitParams,
    options?: RequestOptions,
  ): APIPromise<ReferenceSubmitResponse> {
...
export interface ReferenceSubmitResponse {
  data: Array<ReferenceSubmitResponse.Data>;
}

export namespace ReferenceSubmitResponse {
  /**
   * A reference (business or financial) on a DIR, in the customer-facing shape. No
   * internal identifiers are exposed.
   */
...
export interface ReferenceSubmitParams {
  /**
   * Exactly two business references.
   */
  business_references: Array<ReferenceSubmitParams.BusinessReference>;
   ...
   */
  financial_reference: ReferenceSubmitParams.FinancialReference;
}

export namespace ReferenceSubmitParams {
  /**
   * One reference supplied at submit. The reference type is implied by the field
   * that carries it (business_references vs financial_reference).
   */
