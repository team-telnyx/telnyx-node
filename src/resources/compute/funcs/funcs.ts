...
import { Export, ExportCreateParams, FuncLogExportConfigResponse } from './export';
...
export class Funcs extends APIResource {
   ...
   * @example
   * ```ts
   * const response = await client.compute.funcs.retrieveLogs(
   *   'id',
   * );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.compute.funcs.retrieveMetricAggregates(
   *     'id',
   *     {
   ...
   * ```ts
   * const response =
   *   await client.compute.funcs.retrieveRevisions('id');
   * ```
   */
  retrieveRevisions(
    id: string,
   ...
   * ```ts
   * const response =
   *   await client.compute.funcs.retrieveShipInspection('id');
   * ```
   */
