...
describe('IpConnections Resource', function () {
  ...
  describe('list', function () {
    ...
    test('Sends the correct request', function () {
      // @ts-expect-error TODO: import .d.ts files under src/test folder
      return telnyx.ipConnections.list().then(listResponseFn);
    });

    test('Sends the correct request [with specified auth]', function () {
      // @ts-expect-error TODO: import .d.ts files under src/test folder
      return telnyx.ipConnections.list(TEST_AUTH_KEY).then(listResponseFn);
    });
  });
      ...
      test('Sends the correct request', function () {
        ...
        return telnyx.ipConnections
          ...
          .then(function (response: ResponsePayload) {
            ...
            // @ts-expect-error TODO: import .d.ts files under src/test folder
            return ipConnection.del().then(responseFn);
          });
      });
      test('Sends the correct request [with specified auth]', function () {
        // @ts-expect-error TODO: import .d.ts files under src/test folder
        return telnyx.ipConnections.retrieve(TEST_UUID).then(function (
          response: ResponsePayload,
        ) {
          const ipConnection = response.data;
          // @ts-expect-error TODO: import .d.ts files under src/test folder
          return ipConnection.del(TEST_AUTH_KEY).then(responseFn);
        });
      });
    });
